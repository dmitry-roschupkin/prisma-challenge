...
Fix some easy errors in code e.g. formating errors (Not all errors can be fixed in this way):
```shell script
npm run lint:fix
```
...
not so readable as it can be, but time was limited, and I decided to not spend the time to prepare perfect test data.
Stress test, tests for large data etc., weren't done too.
...
brackets. Something like: `FILTER f1 and (f2 or f3 of f4)` in this case we will have one filter `f1` and one multiple
...
- To implement ordering without implementation of `indexes` - we can:
  1. Accumulate result data, and each new founded/filtered record have to be inserted in ordered place in results
     data. In this case result data will be every time ordered. Complexity from ~Q(n*log(n)) up to ~Q^2 (depending 
